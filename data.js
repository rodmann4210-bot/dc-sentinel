// api/analyze.js — AI composite analysis
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!scores || Object.keys(scores).length === 0) return res.status(400).json({ error: "No signal data provided" });

    const lines = Object.entries(scores)
      .map(([k, s]) => `  • ${k.replace(/_/g, " ")}: ${s.value} → ${s.z >= 0 ? "+" : ""}${s.z}σ [${s.label}]`)
      .join("\n");

    const qualLines = qualActive
      ? Object.entries(qualActive).filter(([, v]) => v).map(([k]) => `  • ${k.replace(/_/g, " ")}: ACTIVE (+2.5σ)`).join("\n")
      : "";
